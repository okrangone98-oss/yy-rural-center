...
import { acceptChatRoom, getChatRoom } from '@/lib/chat-store';
...
export async function POST(
  ...
  try {
    ...
    if (session.user.role !== 'ADMIN' && room.instructorEmail !== email) {
      return NextResponse.json({ success: false, message: '문의 수락 권한이 없습니다.' }, { status: 403 });
    }

    if (room.status !== 'PENDING') {
      return NextResponse.json({ success: false, message: '대기 중인 문의만 수락할 수 있습니다.' }, { status: 400 });
    }

    await acceptChatRoom(params.roomId);
    return NextResponse.json({ success: true, message: '문의가 수락되어 채팅을 시작할 수 있습니다.' });
  } catch (acceptError) {
    if (isFirebaseTimeoutError(acceptError)) {
    ...
    return NextResponse.json(
      {
        success: false,
        message: acceptError instanceof Error ? acceptError.message : '문의 수락에 실패했습니다.',
      },
      { status: 400 },
    );
  }
